...
class L2Tester(LayerImportanceTester):
    ...
    def get_distance(self, reduce_fn=np.mean, 
                 excluded=['norm', 'bn'], included=['weight', 'bias']):
        ...
        for layer in self.get_target_modules():
            org_state = OrderedDict([(k,v) for k,v in state1.items() 
                                     if layer in k  and check_type(k, excluded=excluded, included=included)])
...
def check_type(_key, excluded=['norm', 'bn'], included=['weight', 'bias']):
    check1 =  not bool(sum([1 if _k in _key.split('.')[-2] else 0  
                         for _k in excluded]))
    
    check2 =  bool(sum([1 if _k in _key.split('.')[-1] else 0  
                         for _k in included]))    
    return check1 and check2

def get_block_dist(org_state, fin_state, reduce_fn=np.mean, norm_n_params=True):
    assert org_state.keys() == fin_state.keys()
    L2s = []
    for l_key in org_state.keys():
        w_org = org_state[l_key].numpy()
        w_fin = fin_state[l_key].numpy()
